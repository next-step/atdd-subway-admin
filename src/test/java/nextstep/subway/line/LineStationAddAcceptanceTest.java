...
public class LineStationAddAcceptanceTest extends AcceptanceTest {
    ...
    void 지하철노선_지하철역_정렬_검증() {
        ...
        지하철_노선에_지하철역_순서_정렬됨(response, Lists.newArrayList(잠실역ID, 강동구청역ID, 몽촌토성역ID));
    }
    ...
    void 상행_종점_역_등록() {
        ...
        // then
        지하철_구간_요청_응답_검증(response, HttpStatus.OK);
        지하철_노선에_지하철역_순서_정렬됨(response, Lists.newArrayList(강동구청역ID, 잠실역ID, 몽촌토성역ID));
    }
    ...
    void 하행_종점_역_등록() {
        ...
        // then
        지하철_구간_요청_응답_검증(response, HttpStatus.OK);
        지하철_노선에_지하철역_순서_정렬됨(response, Lists.newArrayList(잠실역ID, 몽촌토성역ID, 강동구청역ID));
    }
