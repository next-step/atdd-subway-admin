...
class SectionAcceptanceTest extends AcceptanceTest {
    ...
    class 삭제 {
        ...
        class 다구간 {
             ...
             */
            @Test
            @DisplayName("종점역을 제거하면 종점역 바로 다음 혹은 바로 이전 역이 종점이 된다.")
            void 종점역_삭제() {
                // When 강남역 제거
                ExtractableResponse<Response> response = 구간_삭제(노선, 강남역);
             ...
             * Scenario: 중간역을 제거한다.
             *   When 중간역을 제거하면
             *   Then 제거된 역의 이전역과 다음역으로 재배치한다.
             */
            @Test
            @DisplayName("중간역을 제거하면 제거된 역 앞과 뒤의 역으로 구간을 병합한다.")
            void 중간역_삭제() {
                // When 역삼역 제거
        ...
        private ExtractableResponse<Response> 구간_삭제(Long lineId, Long stationId) {
            return RestAssured.given().log().all().when()
                    .delete(String.format("/lines/%d/sections?stationId=%d", lineId, stationId)).then().log().all()
                    .extract();
        }
    }

    private ExtractableResponse<Response> 구간_추가(SectionRequestDto sectionRequestDto, Long lineId) {
        return RestAssured.given().log().all().body(sectionRequestDto).contentType(MediaType.APPLICATION_JSON_VALUE)
                .when().post(String.format("/lines/%d/sections", lineId)).then().log().all().extract();
    }
