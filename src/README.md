...
* [x] 지하철 노선 추가 기능을 인수 조건을 기반으로 구현
* [x] 지하철 노선 추가 인수 조건을 예외 케이스를 포함한 인수 테스트를 작성
* [x] 지하철 노선의 구간 삭제 기능을 인수 조건을 기반으로 구현
* [x] 지하철 노선의 구간 삭제 인수 조건을 예외 케이스를 포함한 인수 테스트를 작성
...
* 중간역이 제거될 경우 재배치를 한다
    * `A --4m-- B --3m-- C` 에서 B 를 제거한 경우 `A --7m-- C` 가 된다
    * 중간역이 제거된 경우 두 구간의 거리는 합이 된다
* 구간에 등록되지 않은 역을 제거할 경우 예외가 발생해야 한다
* 구간이 하나인 노선에서 역을 제거할 경우 예외가 발생해야 한다
