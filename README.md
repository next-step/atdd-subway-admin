...
* 노선 생성 시 종점역(상행, 하행) 정보를 요청 파라미터에 함께 추가
    * 두 종점역은 구간(`Section`)의 형태로 관리되어야 함
        1. [ ] 인수테스트(`LineAcceptanceTest`)  수정  
        2. [ ] `Section` 클래스 생성 : 출발역, 도착역, 거리 변수포함
        3. [ ] `LineRequest`에 변수 추가 : 출발역 Id, 도착역 Id, 거리 
        4. [ ] `Line`과 `Section`간 연관관계 추가 
* 노선 조회 시 응답 결과에 역 목록 추가하기
    * 상행역부터 하행역 순으로 정렬되어야 함
        1. [ ] `Line`과 `Section`간 연관관계 추가
