...
- 객체의 equals에 대해서 
  - Entity class의 equals는 id만을 비교해야한다
    - 동등성 비교를 의미하고 동등성 비교라는 것은 두 객체의 레퍼런스가 다라도 같은 것임을 어떻게 보장할 것인가? 
    - 1차 캐시를 초기화한 후 다시 데이터베이스에서 동일한 데이터를 읽어온 후 새로운 객체가 생성되어도 두 객체는 동등성을 보장해야하는 부분
  - 하지만 위와 같이 id만을 비교하게 된다면 `일반적인 객체`의 비교와 다르게 된다. 
    - entity에서 equals를 사용하기 보다는 entity를 비교할 일이 있다면 아예 id를 명시적으로 가져와서 비교하는 것이 좋다고 생각한다.
  - 준영속성 객체인 경우에는 Id 값이 null 이다 이 경우도 생각해볼 문제이다.
  - 만약 
...
- [x] 구간 삭제시 예외 케이스를 고려하기 

## 요구사항 설명 
노선의 `구간을 제거`하는 기능을 구현하기
...
- 고아 객체에 대한 설정, OneToMany의 경우 기본 전략이 LAZY이기에 생략 가능!
- 매직넘버 사용을 자제하라
