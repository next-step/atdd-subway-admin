...
- 지하철 구간 (Section)
    - [ ] 지하철 노선의 구간을 생성한다.
        - [x] 구간으로 등록할 지하철 역이 없는 경우 지하철 구간 생성에 실패한다.
        - [ ] 같은 지하철 구간이 이미 등록되어 있는 경우 지하철 구간 생성에 실패한다.
        - [x] 구간의 거리가 최소 거리 이하인 경우 지하철 구간 생성에 실패한다.
        - [ ] 기존 역 사이의 길이보다 크거나 같은 경우 지하철 구간 생성에 실패한다.
    - [ ] 지하철 구간 목록을 조회한다.
...
- 지하철 구간
    ...
    Feature: 지하철 구간 관리 기능
        ...
        Scenario: 지하철 구간 생성이 실패한다.
            Given 같은 지하철 구간이 이미 등록되어 있다.
            When 지하철 구간을 생성 요청한다.
            Then 지하철 구간 생성이 실패한다.
  
        Scenario: 지하철 구간 생성이 실패한다.
            Given 지하철 구간의 거리가 최소 거리 이하이다.
            When 지하철 구간을 생성 요청한다.
            Then 지하철 구간 생성이 실패한다.
  
        Scenario: 지하철 구간 생성이 실패한다.
            Given 기존 역 사이의 길이보다 크거나 같다.
            When 지하철 구간을 생성 요청한다.
            Then 지하철 구간 생성이 실패한다.
