...
* 연결된 역과 역 구간 사이에 구간 추가
  * 구간 생성 요청 상행 역과 기존에 상행역이 일치할 경우
    * [ ] 구간 추가
    * [ ] 기존 구간의 상행역을 입력받은 하행역으로 수정
  * 구간 생성 요청 하행 역과 기존에 하행역이 일치할 경우
    * [ ] 구간 추가
    * [ ] 기존 구간의 하행역을 입력받은 상행역으로 수정
  * `기존 구간 거리 = 기존 구간 거리 - 입력 받은 구간 거리`으로 구간 업데이트
    * [ ] 업데이트한 구간의 길이는 최소 1이상이어야 함
    * [ ] 1미만일 경우 잘못된 구간 설정에 대한 예외 처리
* 상행 종점 구간 및 하행 구간 추가
  * [ ] 구간 생성 요청 하행역과 상행 종점역이 일치할 경우
  * [ ] 구간 생성 요청 상행역과 하행 종점역이 일치할 경우
