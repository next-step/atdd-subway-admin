...
- [x] 인수 테스트 구현
  - [x] 지하철 역 삭제
    ...
    - [x] 노선의 역 중 중간 역 제거
    - [x] 노선의 역 중 마지막 역 제거
    - [x] 노선의 구간이 하나 뿐일때 제거 불가 예외 처리
    - [x] 노선의 구간에 속하지 않는 역으로 제거 요청할 때 예외 처리
...
## 피드백 반영 목록
- [x] 유효성 검사간 실패시 예외를 던지도록 수정
- [x] 머지 불가 구간 예외에 적절한 메시지를 남기도록 수정
- [x] 두 구간을 병합하는 메소드의 이름을 적절하게 변경
- [x] Sections 내 미사용 size메소드의 접근지정자를 Sections 내부에서만 쓰므로 size()로 변경
- [x] 불필요한 테스트용 목적의 코드 제거
- [x] Sections 도메인 테스트 클래스에서 구간 삭제에 관련된 테스트를 추가적으로 구현
...
- [x] Stations 내 stream 반환 로직 제거
- [x] 구간 추가에서 사용되는 여러 메소드에 단위 테스트 구현
