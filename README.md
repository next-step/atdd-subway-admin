...
- [ ]  인수 조건 : 구간이 하나인 노선에서 마지막 구간을 제거할 때 제거되지 않는다.
  * Given : 노선에 A-B 역이 존재한다.
  * When : A역을 제거한다.
