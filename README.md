...
7. [패키지 의존관계에 대하여](https://www.youtube.com/watch?v=dJ5C4qRqAgA)
    1. 패키지 안에 같이 둬야하는가?
       - 함께 생성되는 삭제되는 것은 묶는다.
       - 같은 도메인의 제약사항을 갖는다면 묶는다.
    2. 추상화에 의존 시킨다. DIP
    3. 이벤트를 발행시켜 도메인에 불필요한 로직을 걷어내고 객체에게 하나의 역할만을 부여할 수 있다.
...
## 2단계 피드백 추가 사항
> 추상화에 의존시켜 라이프사이클 문제를 해결할 수 있지 않을까? 하는 생각이들었지만 추상화에 의존한다는 것은 서로 간의 느슨한 결합을 만들고 응집도가 낮아지지 않을까 생각하게 되었습니다.

1. 설계에 대해서



답변 : 응집도란 변경이 발생할 때 모듈 내부에서 발생하는 **변경의 정도로 측정**이 가능한데   
추상화를 통해 분리하여 **여러 모듈을 수정**해야한다면 오히려 추상화를 통해 응집도가 낮아진다면 잘못된 추상화일 가능성이 있습니다.

> 만약 지하철라인에 대한 도메인 지식이 커지고 Line에대한 도메인 지식 커지게 된다면 그 때는 쪼개야 하는 순간이라고 생각이 듭니다. 
> 만약, 쪼개야 한다면 두 패키지 간의 라이프사이클 해결하기 위해 도메인 이벤트를 생각할 수 있지 않을까? 하는 생각이 드네요

답변 : 제 개인적인 생각으로는 LIne 도메인이 확장되더라도 Section과 분리되지는 않을 것 같다는 의견입니다.   
그 이유는 지하철 노선 구간(Section)이러는 것은 지하철 노선(Line)에 의해 관리가 되기 때문입니다.

답변 : Section의 생명주기는 Line을 통해 생기고 있다   
주문을 예시로 들면 Order와 OrderItem의 관계와 유사할 수도 있을 것 같습니다.

2. 직접 주입하는 것보다 addAll 더 좋다.
> this.sections.addAll(sections);

3. 어떠한 방식으로 찾아 낼 수 있을까? 
여러개의 Section에서 상행역부터 하행역을 순서대로 정렬할 때,
최상위 상행역을 찾고 연결된 하행역들을 순서대로 찾아보는 방법도 있을 것 같습니다

## 3단계 - 구간 추가 기능

### 요구사항 설명 
1. 지하철 구간 등록 인수 테스트 작성과 기능 구현
2. 역 사이에 새로운 역을 등록할 경우 
- 새로운 길이를 뺀 나머지를 새롭게 추가된 역과의 길이로 설정 
- 예를 들어서 A와 C역이 10m 인경우 A와 B역 4m 추가시 
  - A와 B의 거리는 4m , A와 C 거리는 6m 이다.
- 새로운 역을 상행 종점으로 등록할 경우
- 새로운 역을 하행 종점으로 등록할 경우
- 강남역은 신분당선에 등록되어있으면서 동시에 신분당선에도 등록될 수 있음
...
- 상행역과 하행역 둘 중 하나도 포함되어있지 않으면 추가할 수 없음
  - 상행역이 해당 노선에 등록되어 있지 않거나 
  - 하행역이 해당 노선에 등록되어 있지 않은경우 등록 할 수 없음 
...
- [ ] 역과 역 사이에 새로운 구간을 등록
  ...
  - 예외
    - [x] 추가 되는 구간 역은 `상행역` 또는 `하행역` 하나는 존재해야한다 없다면 예외처리
    - [x] `상행역`과 `하행역`이  이미 노선에 등록되어 있다면 예외
    - [x] 거리가 기존 역과의 거리 보다 크거나 같으면 예외
    
- [x] 지하철역은 다음역의 순서대로 조회가 가능해야함 
