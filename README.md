...
### 시나리오 Todo-list
  ...
  - [ ] 시나리오2
    - [ ] Sections에서 상행 종점역 구간을 찾는 기능 구현
        - [ ] 중복이 있는 전체역 중 상행 역이 한번밖에 없는 경우를 찾는다.
    - [ ] Line에서 새로운 구간을 추가하기 전에 신규 구간이 상행 종점역 교체인지 확인하도록 기능 구현
      - [ ] 상행 종점역 등록일 경우 상행 종점역 등록 절차 진행
      - [ ] 상행 종점역 등록이 아닐 경우 일반 구간 등록 절차 진행
