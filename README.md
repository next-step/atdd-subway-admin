...
* Feature: 구간 추가 기능
    ...
    * [ ] Scenario: 구간 사이에 새로운 역을 등록 시 구간거리가 기존 역사이의 구간 거리와 같은 값으로 역 등록.
        * given: 양재역, 정자역이 등록되어 있음.
        * given: 양재역, 정자역 구간이 포함된 신분당선 노선이 등록되어 있음.
        * given: 판교역이 등록되어 있음.
        * when: 양재역, 판교역 구간 등록을 요청한다. 등록 시 구간 거리를 양재역, 정자역 구간 거리와 같은 을 입력한다.
        * then: 양재역, 판교역 구간 등록이 실패한다.
    * [ ] Scenario: 구간 사이에 새로운 역을 등록 시 구간거리가 기존 역사이의 구간 거리보다 큰값으로 역 등록.
        * given: 양재역, 정자역이 등록되어 있음.
        * given: 양재역, 정자역 구간이 포함된 신분당선 노선이 등록되어 있음.
        * given: 판교역이 등록되어 있음.
...
* [x] 기능 구현
    * [x] 구간 추가 기능 개발
        ...
        * [x] 새로운 역을 하행 종점으로 등록
            ...
            * [x] 신규 구간 저장
    * [ ] 구간 추가 기능 예외 처리
        * [ ] 구간 사이에 새로운 역을 등록 시 구간거리가 기존 역사이의 구간 거리 보다 크거나 같은 값 입력 시 예외 처리
        * [ ] 이미 등록된 구간 등록 요청 시 예외 처리
        * [ ] lineStations에 상행역과 하행역 둥중 하나도 포함되어 있지 않으 경우 예외 처리
